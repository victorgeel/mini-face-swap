...
import os
from huggingface_hub import hf_hub_download # ဒါက HF ကနေ ဒေါင်းဖို့ပါ

# Mac (CoreML) အတွက် Provider Setting
...
if not os.path.exists(model_file):
    ...
    try:
        # Hugging Face မှ တိုက်ရိုက်ဆွဲယူမည့် လမ်းကြောင်း
        hf_hub_download(
...
def process_image(source_img, target_img):
    ...
    if len(source_faces) == 0: return target_img
...
with gr.Blocks(title="Mini Face Swap (Mac)") as demo:
    gr.Markdown("# 🚀 Mini Face Swap (Auto-Download Model)")
    gr.Markdown("Note: ပထမဆုံးအကြိမ် Run လျှင် Model ဒေါင်းလုပ်ဆွဲချိန် ခေတ္တစောင့်ပေးပါ။")
    
    with gr.Tabs():
        with gr.TabItem("🖼️ Photo Swap"):
            with gr.Row():
                p_src = gr.Image(label="Source Face", type="numpy")
                p_tgt = gr.Image(label="Target Image", type="numpy")
                p_out = gr.Image(label="Result")
            gr.Button("Swap Photo").click(process_image, inputs=[p_src, p_tgt], outputs=p_out)
        ...
        with gr.TabItem("🎥 Video Swap"):
            ...
            gr.Button("Swap Video").click(process_video, inputs=[v_src, v_tgt], outputs=v_out)
